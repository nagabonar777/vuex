<template>
    <div>
        ...
        <label> Number</label>
        <input v-model="newUser.address.number" type="text" required><br><br>
        <label> Zipcode</label>
        ...
        <button type="submit" class="grid cursor-pointer select-none rounded-md border border-indigo-500
        bg-indigo-500 py-2 px-5 text-center align-middle text-sm text-white shadow hover:border-indigo-600
        hover:bg-indigo-600 hover:text-white focus:border-indigo-600 focus:bg-indigo-600
        focus:text-white focus:shadow-none"> Create User</button>
    </form>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...mapActions("User", ["Create", "setNotification"]),
        async Create() {
            try {
                console.log("Data", this.newUser);
                       const newUser = await this.Create(this.userData);
